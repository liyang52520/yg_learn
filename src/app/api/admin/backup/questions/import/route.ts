  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (file.name.endsWith(".zip")) {
        const tmpPath = path.join(process.cwd(), "backups", `__questions_import_temp.zip`);
    ...
    if (body.version !== 1 || body.type !== "questions" || !body.data) {
      return NextResponse.json({ error: "备份文件格式无效" }, { status: 400 });
    }

    const { questionCategories, questions } = body.data;
    if (!Array.isArray(questionCategories) || !Array.isArray(questions)) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const cat of questionCategories) {
        ...
        const upserted = await tx.questionCategory.upsert({
          where: { slug: cat.slug },
          create: catData,
          update: catData,
        });
      ...
      // Upsert questions by title with remapped categoryId
      let qCount = 0;
      for (const q of questions) {
        const { id, createdAt, updatedAt, categoryId, ...qData } = q;
        await tx.question.upsert({
          where: { title: q.title },
          create: { ...qData, categoryId: catIdMap.get(categoryId) ?? categoryId },
          update: { ...qData, categoryId: catIdMap.get(categoryId) ?? categoryId },
        });
        qCount++;
      }

      return { questionCategories: catCount, questions: qCount };
    });
